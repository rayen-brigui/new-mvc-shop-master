...
function pagination(string $url, $page, $total): string
{
    ...
    //first
    if ($page == 1) {
        $out .= '<li class="disabled"><span>Head</span></li>';
    } else {
        $out .= '<li><a href="' . $url . '">Head</a></li>';
    }
...
function upload($field, $config = [])
{
    ...
    $options = array_merge($options, $config); //Merge one or more arrays
    if (!isset($_FILES[$field])) return FALSE;
    $file = $_FILES[$field];
    if ($file['error'] != 0) return FALSE;
    $temp = explode(".", $file["name"]);
    $ext = end($temp); //Set an array's internal pointer to its last element
...
function adminPagination(string $url, $page, $total): string
{
    ...
    //first
    if ($page == 1) {
        $out .= '<li class="page-item disabled"><span class="page-link">Head</span></li>';
    } else {
        $out .= '<li class="page-item"><a style="color: blueviolet" class="page-link" href="' . $url . '">Head</a></li>';
    }
